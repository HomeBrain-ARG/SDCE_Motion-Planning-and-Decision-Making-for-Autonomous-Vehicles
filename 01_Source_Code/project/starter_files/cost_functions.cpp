...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      auto circle_center_x = CIRCLE_OFFSETS[c] * std::cos(cur_yaw) + cur_x;  // <- Updated
      auto circle_center_y = CIRCLE_OFFSETS[c] * std::sin(cur_yaw) + cur_y;  // <- Updated
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double dist = std::sqrt(std::pow((circle_center_x-actor_center_x), 2) + std::pow((circle_center_y-actor_center_y), 2)); //<- Updated
